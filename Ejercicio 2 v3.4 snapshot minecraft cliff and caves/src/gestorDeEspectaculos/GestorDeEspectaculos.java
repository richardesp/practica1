...
public class Main {
	...
	static private void menuUsuarioLogeado() {
		...
		while (true) {
			...
			System.out.print("[8] Vender entradas a una sesión\n");
			...
			case 8:
				...
				try {
					miFecha = new SimpleDateFormat("dd/MM/yyyy HH:mm").parse(strFecha);
					gestorEspectaculos.venderEntradas(titulo, miFecha);
				} catch (Exception e) {
					System.err.print(e);
				}
	...
	private static void menuAdminLogeado() {
		...
		while (true) {
			...
			System.out.print("[9] Ver espectáculos próximos\n\n");
			System.out.print("[10] Cerrar sesión\n\n"); // Ok !
			System.out.print("[11] Cerrar programa\n\n"); // Ok !
			...
			case 9:
				int n;
				try {
					n=gestorEspectaculos.getEspectaculos().size();
				for(int i=0; i<n;i++) {
					for(int j=0;j<gestorEspectaculos.getEspectaculos().get(i).getSesiones().size();j++) {
						Date fechaActual = new Date();
						if(gestorEspectaculos.getEspectaculos().get(i).getSesiones().get(j).getFecha().getTime() > fechaActual.getTime()) {
						System.out.println("\nEspectáculo :" + gestorEspectaculos.getEspectaculos().get(i).getTitulo() + "\nFecha sesión: " + gestorEspectaculos.getEspectaculos().get(i).getSesiones().get(j).getFecha() + "\nLocalidades disponibles: " + gestorEspectaculos.getEspectaculos().get(i).getSesiones().get(j).getLocalidades());							
						}
					}
				}
			} catch (Exception f) {
